...
/**
 * Isolation test B: provides {@link Greeting} as "BBB" via inline field.
 * Runs alongside {@link InlineProducerIsolationAaaTest} which provides "AAA"
 * for the same type — proving inline producers are scoped per test class.
 */
@EnableTestBeans
class InlineProducerIsolationBbbTest {

    @Produces
    @TestBean
    private static Greeting greeting = name -> "BBB";
    ...
    @Test
    @DisplayName("Inline producer B returns 'BBB'")
    void inlineProducerReturnsBBB() {
        assertEquals("BBB", consumer.getGreeting().greet("x"));
    }
}
