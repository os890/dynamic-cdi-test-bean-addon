...
/**
 * Verifies the README snippet: "Inline producer fields".
 */
@EnableTestBeans
class InlineProducerSnippetTest {

    @TestBean
    private static Greeting greeting = createMock();

    @Inject
    GreetingConsumer consumer;

    private static Greeting createMock() {
        Greeting mock = Mockito.mock(Greeting.class);
        Mockito.when(mock.greet("world")).thenReturn("Stubbed!");
        return mock;
    }

    @Test
    @DisplayName("README: @TestBean on static field provides inline mock")
