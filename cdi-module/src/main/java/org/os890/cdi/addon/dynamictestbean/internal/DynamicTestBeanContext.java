 ...
 *
 * <p>All access is synchronized on the class object to prevent race
 * conditions when multiple test classes are processed. Note that
 * parallel test execution across classes is <strong>not supported</strong>
 * — each test class must run sequentially.</p>
 *
...
public final class DynamicTestBeanContext {
    ...
    private static Class<?> activeTestClass;
    private static boolean addTestClass = true;
    private static boolean limitToTestBeans;
    private static boolean manageContainer = true;

    private DynamicTestBeanContext() {
    }

    /** Sets the active test class. */
    public static synchronized void setActiveTestClass(Class<?> testClass) {
        activeTestClass = testClass;
    }

    /** Returns the active test class, or {@code null} if none. */
    public static synchronized Class<?> getActiveTestClass() {
        return activeTestClass;
    }

    /** Sets whether the test class should be added as a CDI bean. */
    public static synchronized void setAddTestClass(boolean value) {
        addTestClass = value;
    }

    /** Returns whether the test class should be added as a CDI bean. */
    public static synchronized boolean isAddTestClass() {
        return addTestClass;
    }

    /** Sets whether to veto all beans except those declared via @TestBean. */
    public static synchronized void setLimitToTestBeans(boolean value) {
        limitToTestBeans = value;
    }

    /** Returns whether whitelist mode is active. */
    public static synchronized boolean isLimitToTestBeans() {
        return limitToTestBeans;
    }

    /** Sets whether the extension manages the CDI container lifecycle. */
    public static synchronized void setManageContainer(boolean value) {
        manageContainer = value;
    }

    /** Returns whether the extension manages the CDI container lifecycle. */
    public static synchronized boolean isManageContainer() {
